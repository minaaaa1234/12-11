...
class RectPanel extends JPanel implements ActionListener, MouseListener, MouseMotionListener {
    ...
    //마우스 눌릴 때마다 위치 저장 변수 초기화하는 메소드 그리고 첫번째 눌린곳 위치 저장
    ...
    //마우스 클릭이 끝났을 때 두번째 눌린곳 위치 저장 후그림그리기 ==> 이부분을 통신으로 그리면 될것같음
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource().getClass().toString().contains("JButton")) {
            shapeString = e.getActionCommand();//선, 네모 ,원 중 어떤건지
        }

        else if (e.getSource().equals(colorComboBox)) {
            colors = (Color) colorComboBox.getSelectedItem(); // color선택
        }

        else if (e.getSource().equals(strokeComboBox)) {
            stroke = (float) strokeComboBox.getSelectedItem(); //굵기 설정
        }
    ...
    public void updatePaint() {
       ...
       minPointx = Math.min(firstPointer.x, secondPointer.x);
        ...
        g.dispose();
        repaint();//다 그렸으면 다시 그리게
    }

    //paintComponent메소드를 통해서 그려왔던거 그림을 액자에 넣는 것이다.
